...
import id.antasari.p5_datasave_230104040222.data.prefs.Prefs
import id.antasari.p5_datasave_230104040222.data.prefs.PreferencesRepository
...
fun SharedPrefsScreen(onDarkChanged: (Boolean) -> Unit = {}) {
    val ctx = LocalContext.current              // <- ambil sekali di sini (AMAN)
    val vm: SharedPrefsViewModel = viewModel(factory = SharedPrefsViewModel.factory(ctx))
    val scroll = rememberScrollState()

    // (opsional) muat preview saat masuk - gunakan ctx yang sudah diambil di atas
    ...
    Column(
        ...
        Surface(shape = MaterialTheme.shapes.large, tonalElevation = 1.dp) {
            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                Row(
                    ...
                    Switch(checked = vm.remember, onCheckedChange = { vm.remember = it })
                }
                ...
                // Tombol - TANPA ikon; label singkat: SAVE, READ, CLEAR
                Row(
        ...
        // ===== Saved Data Preview - FULL lebar, label kiri & isi kanan =====
        vm.preview?.let { p ->
            Surface(
